...
function pod(overrides = {}) {
    return {
        metadata: {
            name: 'berth-ws-edkief-repo-0badf00d',
            annotations: {
                'berth.kieffer.me/repo-full-name': 'edkief/repo',
                'berth.kieffer.me/branch': 'feature/x',
                'berth.kieffer.me/repo-url': 'git@github.com:edkief/repo.git',
            },
            ...overrides.metadata,
        },
        spec: {
            containers: [{ resources: { limits: { cpu: '2', memory: '4Gi' } } }],
            volumes: [{ name: 'ws', persistentVolumeClaim: { claimName: 'berth-ws-edkief-repo-0badf00d' } }],
        },
...
test('pod manifest: labels are k8s-legal and slashes live in annotations', () => {
    const m = buildWorkspacePodManifest({
        id: 'berth-ws-edkief-repo-0badf00d',
        ...
        pvcName: 'berth-ws-edkief-repo-0badf00d',
    });
    ...
    assert.equal(m.metadata.labels['berth.kieffer.me/branch'], 'feature-nested-name');
    assert.equal(m.metadata.annotations['berth.kieffer.me/branch'], 'feature/nested/name');
});
...
test('pod manifest: a selected resource profile controls requests and limits', () => {
    ...
    const m = buildWorkspacePodManifest({
        ...
        repoUrl: 'git@github.com:e/r.git', repoFullName: 'e/r',
        branch: 'main', baseBranch: 'main', sessionName: 'r-main',
        pvcName: 'berth-ws-x-0badf00d', resourceProfile: 'large', resources,
    });
    assert.deepEqual(m.spec.containers[0].resources, resources);
    assert.equal(m.metadata.annotations['berth.kieffer.me/resource-profile'], 'large');
});
